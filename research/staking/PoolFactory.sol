...
// File: IcecreamPad/contracts/Presale/contracts/interfaces/IPool.sol
interface IPool {
  function initialize(
...
// File: IcecreamPad/contracts/Presale/contracts/PoolFactory.sol
pragma solidity ^0.8.0;
...
contract PoolFactory is Ownable {
    ...
    function changelock(address _lock) external onlyOwner {
        lock = _lock;
    }
    function removePoolForToken(address token, address pool) external {
        require(msg.sender == owner() || poolToToken[msg.sender] == token, "remove token not allowed");
