...
contract BansclawNftMarket is Initializable, ERC1155Upgradeable,ERC1155BurnableUpgradeable, OwnableUpgradeable , ERC1155PausableUpgradeable, ERC1155SupplyUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable  {
    ...
    */
    event MintBurnEmit(address to, uint256 id, uint256 amount);
    ...
    */
        ...
        payable
    {
       uint id = structs[catId][catIdTokId].tokenId;
        ...
        require(_totalSupply < structs[catId][catIdTokId].supply && msg.value >= (structs[catId][catIdTokId].cost * amount),  "The  total supply you are wanting to purchase needs to be less than total supply!");
    ...
    */
        ...
        payable
    {
        ...
        require(id > 0 && id <= tokenId, "Invalid token id sent!");
        require(structs[catId][catIdTokId].paused, "This token is currently paused!");
        require(structs[catId][catIdTokId].airdrop, "This token isn't set as an!");
        

        address sender = airdropAddress;
        uint _totalSupply = totalSupply(id);

        require(_totalSupply < structs[catId][catIdTokId].supply && msg.value >= (structs[catId][catIdTokId].cost * amount), "");
        uint totAttemptPurchase =(structs[catId][catIdTokId].cost * amount);
        require(_totalSupply < totAttemptPurchase, "The  total supply you are wanting to purchase needs to be less than total supply!");
     ...
     /*
    * mint and burn those tokens.   have to send catid, id + cost
    */
    function mintBurnTokens(uint256 catId, uint256 catIdTokId, uint256 amount, address airdropAddress) onlyRole(ADMIN_ROLE)
        public
        payable
    {
        uint id = structs[catId][catIdTokId].tokenId;
        ...
        address sender = airdropAddress;
        uint _totalSupply = totalSupply(id);
        uint _totalSupplyAndAmount = _totalSupply + amount;
        require(_totalSupplyAndAmount > structs[catId][catIdTokId].supply, "Not enough supply available!");        
        
        _burn(sender, id, amount);
        emit MintBurnEmit(sender, id, amount);

        // incremeent
        structs[catId][catIdTokId].itemsSold = structs[catId][catIdTokId].itemsSold + amount;
